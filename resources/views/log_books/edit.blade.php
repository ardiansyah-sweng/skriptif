...
@section('title', 'Edit Log Book Bimbingan')

@section('content')
    <div class="crumb" style="font-size:12px;color:#94a3b8;display:flex;align-items:center;gap:6px;margin-bottom:8px;">
        ...
        <a href="{{ route('log-books.index') }}" style="color:#64748b;text-decoration:none">Log Book Bimbingan</a>
        <i class="fa-solid fa-chevron-right" style="font-size:8px"></i>
        <span class="fw-semibold" style="color:#0f172a;">Edit</span>
    </div>

    <div class="mb-4">
        <h1 class="main-title">Edit Catatan Bimbingan</h1>
        <p class="sub-title">Perbarui catatan konsultasi bimbingan, feedback, atau status verifikasi di bawah ini.</p>
    </div>
    ...
    @if($errors->any())
        <div class="alert alert-danger alert-dismissible fade show mb-4 border-0 shadow-sm" style="background-color: #fee2e2; color: #b91c1c; border-radius: 8px;">
            <i class="fa-solid fa-circle-xmark me-2"></i> <strong>Gagal menyimpan perubahan:</strong>
    ...
    <div class="form-card">
        <form action="{{ route('log-books.update', $logBook->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="lecturer_id" id="lecturer_id" class="form-select @error('lecturer_id') is-invalid @enderror" required>
                    ...
                    @foreach($lecturers as $lecturer)
                        <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $logBook->lecturer_id) == $lecturer->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $logBook->date ? $logBook->date->format('Y-m-d') : '') }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="activity" id="activity" rows="5" class="form-control @error('activity') is-invalid @enderror" required>{{ old('activity', $logBook->activity) }}</textarea>
                <div class="hint">Minimal 5 karakter. Berikan rincian ringkas tapi padat mengenai hasil diskusi bimbingan.</div>
            </div>
            <div class="mb-4">
                <label for="feedback" class="form-label">Feedback / Catatan Dosen (Opsional)</label>
                <textarea name="feedback" id="feedback" rows="3" class="form-control @error('feedback') is-invalid @enderror" placeholder="Feedback, revisi, atau saran perbaikan dari dosen pembimbing (jika ada)...">{{ old('feedback', $logBook->feedback) }}</textarea>
                <div class="hint">Catatan evaluasi atau revisi yang harus dikerjakan mahasiswa.</div>
            </div>
            <div class="mb-4">
                <label for="status" class="form-label">Status Bimbingan <span class="text-danger">*</span></label>
                <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                    <option value="pending" {{ old('status', $logBook->status) == 'pending' ? 'selected' : '' }}>Pending (Menunggu Verifikasi)</option>
                    <option value="approved" {{ old('status', $logBook->status) == 'approved' ? 'selected' : '' }}>Approved (Telah Disetujui)</option>
