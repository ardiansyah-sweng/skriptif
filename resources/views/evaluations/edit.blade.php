...
<body>
    <div class="wrap">
        <div class="page-head">
            ...
            <h1>Edit Evaluasi</h1>
            <p>Perbarui hasil penilaian per unsur</p>
        </div>
        ...
        <form action="{{ route('evaluations.update', $evaluation->id) }}" method="POST" id="evaluationForm">
            ...
            <div class="card">
                ...
                @php $currentRole = old('role', $evaluation->role); @endphp

                <div class="role-tabs">
                    <label class="role-tab {{ $currentRole == 'pembimbing' ? 'active' : '' }}" data-role-tab="pembimbing">
                        <input type="radio" name="role" value="pembimbing" {{ $currentRole == 'pembimbing' ? 'checked' : '' }}>
                        Dosen Pembimbing
                    </label>
                    <label class="role-tab {{ $currentRole == 'penguji' ? 'active' : '' }}" data-role-tab="penguji">
                        <input type="radio" name="role" value="penguji" {{ $currentRole == 'penguji' ? 'checked' : '' }}>
                        Dosen Penguji
                    </label>
                </div>
                @error('role') <div class="error-text" style="margin-bottom:12px">{{ $message }}</div> @enderror

                <div class="role-panel {{ $currentRole == 'pembimbing' ? 'active' : '' }}" data-role-panel="pembimbing">
                    <div class="field" style="margin-bottom:0">
                        ...
                        <select id="lecturer_id_pembimbing" class="lecturer-select" data-role="pembimbing">
                            ...
                            @foreach($lecturers as $lecturer)
                                <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $currentRole == 'pembimbing' ? $evaluation->lecturer_id : null) == $lecturer->id ? 'selected' : '' }}>
                                    {{ $lecturer->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="role-panel {{ $currentRole == 'penguji' ? 'active' : '' }}" data-role-panel="penguji">
                    <div class="field" style="margin-bottom:0">
                        ...
                        <select id="lecturer_id_penguji" class="lecturer-select" data-role="penguji">
                            ...
                            @foreach($lecturers as $lecturer)
                                <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $currentRole == 'penguji' ? $evaluation->lecturer_id : null) == $lecturer->id ? 'selected' : '' }}>
                                    {{ $lecturer->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="card">
                ...
                <div class="total-row">
                    ...
                    <span class="total-value" id="totalDisplay">{{ $evaluation->score }}</span>
                </div>
    ...
    <script>
        ...
        function setActiveRole(role, keepLecturer = false) {
        ...
        // Saat pertama load, pertahankan dosen yang sudah tersimpan untuk peran aktif
        const initialRole = document.querySelector('input[name="role"]:checked')?.value || 'pembimbing';
        setActiveRole(initialRole, true);
