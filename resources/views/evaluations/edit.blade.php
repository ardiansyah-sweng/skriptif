...
<div class="wrap-eval">
    <div class="page-head">
        <h1>Edit Evaluasi</h1>
        <p>Perbarui hasil penilaian per unsur untuk Dosen Pembimbing atau Dosen Penguji</p>
    </div>
    ...
    <form action="{{ route('evaluations.update', $evaluation->id) }}" method="POST" id="evaluationForm">
        ...
        <div class="card-eval">
            <div class="card-title-eval"><i class="ti ti-file-description"></i> Data Skripsi &amp; Tanggal</div>
            ...
            <div class="field" style="margin-bottom:0">
                ...
                <input type="number" name="weight" id="weight" min="0" max="100" step="0.01" value="{{ old('weight', $evaluation->weight) }}">
        ...
        <div class="card-eval">
            <div class="card-title-eval"><i class="ti ti-users"></i> Peran Penilai &amp; Dosen</div>

            <div class="role-tabs">
                <label class="role-tab {{ old('role', $evaluation->role) == 'pembimbing' ? 'active' : '' }}" data-role-tab="pembimbing">
                    <input type="radio" name="role" value="pembimbing" {{ old('role', $evaluation->role) == 'pembimbing' ? 'checked' : '' }}>
                    Dosen Pembimbing
                </label>
                <label class="role-tab {{ old('role', $evaluation->role) == 'penguji' ? 'active' : '' }}" data-role-tab="penguji">
                    <input type="radio" name="role" value="penguji" {{ old('role', $evaluation->role) == 'penguji' ? 'checked' : '' }}>
                    Dosen Penguji
                </label>
            </div>
            ...
            <div class="role-panel {{ old('role', $evaluation->role) == 'pembimbing' ? 'active' : '' }}" data-role-panel="pembimbing">
                <div class="field" style="margin-bottom:0">
                    ...
                    <select id="lecturer_id_pembimbing" class="lecturer-select" data-role="pembimbing">
                        ...
                        @foreach($lecturers as $lecturer)
                            <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $evaluation->lecturer_id) == $lecturer->id && old('role', $evaluation->role) == 'pembimbing' ? 'selected' : '' }}>
                                {{ $lecturer->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            {{-- List Dosen Penguji --}}
            <div class="role-panel {{ old('role', $evaluation->role) == 'penguji' ? 'active' : '' }}" data-role-panel="penguji">
                <div class="field" style="margin-bottom:0">
                    ...
                    <select id="lecturer_id_penguji" class="lecturer-select" data-role="penguji">
                        ...
                        @foreach($lecturers as $lecturer)
                            <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $evaluation->lecturer_id) == $lecturer->id && old('role', $evaluation->role) == 'penguji' ? 'selected' : '' }}>
                                {{ $lecturer->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <input type="hidden" name="lecturer_id" id="lecturer_id" value="{{ old('lecturer_id', $evaluation->lecturer_id) }}">
            @error('lecturer_id') <div class="error-text">{{ $message }}</div> @enderror
        </div>

        <div class="card-eval">
            <div class="card-title-eval"><i class="ti ti-clipboard-list"></i> Komponen Penilaian</div>
                ...
                @foreach($components['pembimbing'] as $component)
                    <div class="component-row">
                        ...
                        <input
                            ...
                            step="0.01"
                            value="{{ old('components.' . $component->id, $existingScores[$component->id] ?? '') }}"
                        >
                    </div>
        ...
        <div class="card-eval">
            <div class="card-title-eval"><i class="ti ti-note"></i> Catatan</div>
            <div class="field" style="margin-bottom:0">
                <textarea name="notes" id="notes" placeholder="Feedback atau catatan tambahan dari dosen">{{ old('notes', $evaluation->notes) }}</textarea>
                @error('notes') <div class="error-text">{{ $message }}</div> @enderror
            </div>
        </div>

        <div class="actions">
            <button type="submit" class="btn-primary-custom">Perbarui Evaluasi</button>
